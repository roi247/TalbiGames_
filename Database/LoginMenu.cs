...
public class LoginMenu : MonoBehaviour {
	...
	void Start ()
    {
		...
		blankErrors();
        StartCoroutine(SetFocusUserameInputfieldRoutine());
	}

    IEnumerator SetFocusUserameInputfieldRoutine()
    {
        yield return new WaitForSeconds(0.35f);
        input_login_username.ActivateInputField();
    }
    ...
    void LoadMainScene(bool allowSceneActivation)
    {
    ...
    void ManageMenuKeysPressed()
    {
        if (Input.GetKeyDown(KeyCode.Tab))
        {
            input_login_password.ActivateInputField();
        }
